...
//[문제]
//학생의 이름, 학년(1-4), 국어점수, 영어점수, 수학점수, 그리고 평균을 가질 수 있는 구조체를 선언하고 
//사용자로부터 이름과 학년, 3과목의 점수를 입력 받는 함수와 구조체를 매개변수로 받아서
//평균을 계산하여 저장하는 함수를 구현하시오. 
//이 때, 학생의 이름은 문자 20자리를 갖는 배열로 선언한다.
